...
- **Local article cabinet:** create, save and switch between multiple articles in the browser.
...
| Persistence   | IndexedDB, LocalStorage and Web Locks                     |
| Build         | Python 3                                                  |
...
IMAGE_TAG="acks-markdown-editor:1.1.0-${VCS_REF:0:7}"
...
### Create, save and reopen articles

1. Select **New** to start with a blank Markdown article; the current article remains in the cabinet.
2. Select **Save** for an explicit local save. Automatic saving remains enabled between manual saves.
3. Open the left **Local article cabinet** to switch between articles or download an individual backup.
4. On mobile, use the sidebar button in the document strip to open the cabinet drawer.

The cabinet uses browser-local IndexedDB. Nothing is uploaded to an ACKS server.
Clearing browser cache or site data, private browsing, browser storage eviction,
or moving to another browser/device can remove local articles. Use **Download
current article** regularly for important work.
...
- Articles and image resources are stored in browser-local IndexedDB; the active-draft
  mirror, themes and recovery metadata use LocalStorage and Web Locks.
- Existing single-draft documents migrate into the article cabinet automatically.
- Manual Save requests persistent storage where supported, but downloadable backups
  remain necessary because browser data is never a server backup.
...
`npm test` rebuilds the single-file app and runs all seven Node test suites.
...
The current release is [`v1.1.0`](https://github.com/shynloc/ACKS-Markdown-Editor/releases/tag/v1.1.0).
